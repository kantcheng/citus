...
static DeferredErrorMessage * RouterPlannableSelectQuery(Query *query);
				 ...
				 PlannerRestrictionContext *plannerRestrictionContext)
{
	...
	distributedPlan->planningError = RouterPlannableSelectQuery(query);
							...
							Oid *distributedTableIdOutput)
{
	...
	if (queryTree->cteList != NIL)
	{
		...
		foreach(cteCell, queryTree->cteList)
		{
			...
			if (cteQuery->commandType == CMD_SELECT)
			{
				DeferredErrorMessage *cteError = RouterPlannableSelectQuery(cteQuery);
				if (cteError)
				{
					return cteError;
				}
...
/*
 * RouterPlannableSelectQuery checks if given query is router plannable,
 ...
 */
static DeferredErrorMessage *
RouterPlannableSelectQuery(Query *query)
{
